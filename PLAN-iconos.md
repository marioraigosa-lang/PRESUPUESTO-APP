...
| B1 | 📝 script listo, sin aplicar | 2026-09-10 | `sql/supabase_iconos_categorias.sql` (BORRADOR). `ADD COLUMN IF NOT EXISTS icono text` en `categorias`, `movimientos` **y `categorias_viaje`** (esta última como adelanto de la fase VIAJE — PASO 3 comentable). Dry-run (PASO 0) de emojis por tabla incluido. Aditivo puro, sin backfill, sin tocar trigger. Falta: revisar y ejecutar en Supabase, luego marcar ✅ aquí y en `sql/README.md`. |
...
| B3 | 📝 script listo, sin aplicar | 2026-09-11 | `sql/supabase_backfill_iconos.sql` (BORRADOR). **Iteración 1** comparaba `CASE emoji WHEN '🛒' THEN ...` con el emoji tipeado literal; el dry-run mostró que casi todo caía en `'tag'`. Diagnóstico con `encode(emoji::bytea,'hex')`: el dato en la BD es UTF-8 canónico correcto — el bug estaba en que varios de los ~450 literales de emoji repetidos en el `.sql` se corrompieron al escribirse (no un problema de selector de variación ni de la BD). **Iteración 2** (actual): el `CASE` compara por `encode(emoji::bytea,'hex')` (ASCII puro, inmune a ese problema) en vez de por el emoji literal; los pares hex→icono se generaron programáticamente desde `src/utils/mapaEmojiIcono.js` (nunca retipeados a mano) y se verificaron con un script que compara las 446 líneas `when` del `.sql` contra el mapa fuente: 0 discrepancias, 0 faltantes. El BLOQUE 5 (trigger) no tenía el bug — cada emoji aparece una sola vez ahí, no en un `CASE` repetido — y se verificó igual por hex, sin cambios. Trae un BLOQUE 0 de solo lectura (`pg_get_functiondef` + dry-run por hex de los 3 backfills) que hay que correr y revisar ANTES de tocar nada — el trigger real en vivo sigue sin verificarse contra la BD (sin acceso desde esta sesión), eso queda pendiente para quien lo ejecute. Falta: revisar con lupa, correr BLOQUE 0, confirmar que el trigger real coincide y que el dry-run por hex ahora sí mapea todo correctamente, ejecutar, marcar ✅ aquí y en `sql/README.md`. |
| B4 | ⬜ pendiente | | |
| B5 | ⬜ pendiente | | |
| B-EXTRA | ⬜ opcional | | |
| VIAJE | ⬜ opcional | | |
