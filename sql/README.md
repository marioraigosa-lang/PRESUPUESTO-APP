...
- ✅ `supabase_fix_borrado_cuentas.sql` — **APLICADO (2026-09-04).** Fase 6 del plan de tarjetas de crédito: corrige una regresión de `supabase_tarjetas_movimientos.sql` (ver ese script en la lista de abajo, no está en el historial numerado tampoco) que hacía fallar el borrado de cualquier cuenta con movimientos. Cambia `movimientos.cuenta_id` y `movimientos.cuenta_destino_id` de `on delete set null` a `on delete cascade`. Verificado con el PASO 0 (nombres reales de constraint) y la prueba funcional del propio script (crear cuenta + gasto + traslado, borrar la cuenta, confirmar que ambos desaparecen sin error) antes de correrlo. No toca `tarjeta_id` (ver los dos scripts siguientes).
...
- ✅ `supabase_borrado_tarjetas_reasignacion.sql` — **APLICADO (2026-09-06).** Modelo final del borrado de tarjetas. (1) FK `movimientos.tarjeta_id`: `on delete cascade` → `on delete restrict` (backstop; la RPC limpia todo antes de borrar la tarjeta). (2) Vista `tarjetas_con_deuda`: agrega `cantidad_gastos`, `total_pagado`, `total_gastado`. (3) Función `eliminar_tarjeta_usuario(p_tarjeta_id, p_cuenta_destino_id)` (SECURITY INVOKER, transaccional): valida deuda calculada = 0; si hay gastos, los reasigna a la cuenta elegida (`tarjeta_id`→null, `cuenta_id`→esa cuenta, conserva monto/fecha/categoría); borra los `pago_tarjeta`; borra la tarjeta. Invariante: como deuda = 0 ⟹ Σgastos = Σpagos, el patrimonio total no cambia. **Verificado:** FK `movimientos_tarjeta_id_fkey` en `ON DELETE RESTRICT`, la vista `tarjetas_con_deuda` expone `cantidad_gastos`/`total_pagado`/`total_gastado`, la función `eliminar_tarjeta_usuario` creada.
