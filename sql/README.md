...
- ✅ `supabase_fix_borrado_cuentas.sql` — **APLICADO (2026-09-04).** Fase 6 del plan de tarjetas de crédito: corrige una regresión de `supabase_tarjetas_movimientos.sql` (ver ese script en la lista de abajo, no está en el historial numerado tampoco) que hacía fallar el borrado de cualquier cuenta con movimientos. Cambia `movimientos.cuenta_id` y `movimientos.cuenta_destino_id` de `on delete set null` a `on delete cascade`. Verificado con el PASO 0 (nombres reales de constraint) y la prueba funcional del propio script (crear cuenta + gasto + traslado, borrar la cuenta, confirmar que ambos desaparecen sin error) antes de correrlo. No toca `tarjeta_id` (sigue en `set null` — mismo bug simétrico ahí, sin resolver, ver el propio script).
