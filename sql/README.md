...
- ✅ `supabase_fix_borrado_cuentas.sql` — **APLICADO (2026-09-04).** Fase 6 del plan de tarjetas de crédito: corrige una regresión de `supabase_tarjetas_movimientos.sql` (ver ese script en la lista de abajo, no está en el historial numerado tampoco) que hacía fallar el borrado de cualquier cuenta con movimientos. Cambia `movimientos.cuenta_id` y `movimientos.cuenta_destino_id` de `on delete set null` a `on delete cascade`. Verificado con el PASO 0 (nombres reales de constraint) y la prueba funcional del propio script (crear cuenta + gasto + traslado, borrar la cuenta, confirmar que ambos desaparecen sin error) antes de correrlo. No toca `tarjeta_id` (ver los scripts siguientes).
- ❌ `supabase_fix_borrado_tarjetas.sql` — **OBSOLETO / SUPERADO por `supabase_borrado_tarjetas_reasignacion.sql`. NO EJECUTAR.** Proponía `on delete cascade` para `movimientos.tarjeta_id` (borrar la tarjeta borraba también sus pagos → el saldo de las cuentas pagadoras "recuperaba" ese dinero). El usuario replanteó el modelo: eso no es correcto. El diagnóstico del bug original (el `set null` rompía `movimientos_traslado_forma_check`) sigue siendo válido y se deja en el archivo como referencia. **Estado real en la BD:** el `on delete cascade` de este script se ejecutó en su momento, pero **ya fue revertido** — `supabase_borrado_tarjetas_reasignacion.sql` lo cambió a `on delete restrict` el 2026-09-06. La FK `movimientos_tarjeta_id_fkey` está hoy en `RESTRICT`.
- ❌ `supabase_borrado_tarjetas_reasignacion.sql` — **OBSOLETO / SUPERADO por `supabase_archivar_tarjetas.sql` (2026-09-07). NO VOLVER A EJECUTAR.** Se aplicó el 2026-09-06 (modelo "borrar reasignando gastos a una cuenta"), pero el usuario replanteó: ahora una tarjeta **nunca se borra, se archiva**. De lo que aplicó este script: la FK `movimientos_tarjeta_id_fkey` en `ON DELETE RESTRICT` **se mantiene** (backstop); la vista `tarjetas_con_deuda` con `cantidad_gastos`/`total_pagado`/`total_gastado` y la función `eliminar_tarjeta_usuario(uuid, uuid)` **se revierten** en `supabase_archivar_tarjetas.sql` (PASO 3 recrea la vista sin esas columnas + filtro `archivada_en is null`; PASO 4 hace `drop function`).
- ⚠️ `supabase_iconos_categorias.sql` — **BORRADOR, NO EJECUTADO (2026-09-10).** Fase B1 del `PLAN-iconos.md` (raíz): unificación visual de iconos (emoji → iconos de línea de lucide). **Aditivo puro:** `add column if not exists icono text` (nullable, sin default) en `categorias`, `movimientos` y `categorias_viaje` — toda fila existente queda en `icono = NULL`. Incluye un PASO 0 de dry-run (solo lectura: emojis actuales y conteo por emoji en las 3 tablas) para armar el backfill después. **No** hace backfill, **no** toca el trigger `handle_new_user()`, **no** borra `emoji` (todo eso es Fase B3 / limpieza posterior), **no** toca RLS ni grants (una columna nueva hereda los privilegios de tabla). `categorias_viaje.icono` va incluida como adelanto de la Fase VIAJE (misma operación sin riesgo; el PASO 3 se puede comentar si se quiere el alcance estricto de B1). Rollback = ignorar la columna (o `drop column if exists icono`). **Cuando se aplique, marcar ✅ y anotar la verificación** (columnas `icono` en `information_schema.columns` = text/nullable/sin default; conteo `icono is not null` = 0).
- ⚠️ `supabase_backfill_iconos.sql` — **BORRADOR, NO EJECUTADO (2026-09-11).** Fase B3 del `PLAN-iconos.md`: backfill de `icono` (agregada en B1) para las filas ya existentes de `categorias`, `movimientos` y `categorias_viaje`, más `create or replace` de `handle_new_user()` agregando la columna `icono` al INSERT de categorías por defecto. **Toca el trigger** — el propio script trae un BLOQUE 0a de solo lectura (`pg_get_functiondef`, misma consulta que `supabase_verificar_trigger.sql`) que hay que correr y comparar a mano contra `supabase_fix_trigger_categorias.sql` (la versión vigente, paso 21) ANTES de ejecutar el resto. **Bug encontrado y corregido en la propia revisión (mismo día):** la primera versión comparaba `CASE emoji WHEN '🛒' THEN ...` con el emoji tipeado literal; el dry-run mostró que la mayoría caía en el fallback `'tag'`. Se diagnosticó con `encode(emoji::bytea, 'hex')` que el dato en la BD es UTF-8 canónico y correcto — el problema estaba en que varios de los ~450 literales de emoji del `.sql` (repetidos 6 veces en distintos bloques) se corrompieron al escribirse. La versión actual compara por **hex UTF-8** (`encode(emoji::bytea, 'hex')`, texto ASCII puro, inmune a ese problema) en vez de por el emoji literal; los pares hex→icono se generaron programáticamente desde `src/utils/mapaEmojiIcono.js` (no se retipeó ningún emoji a mano) y se verificaron con un script que compara cada línea del `.sql` contra el mapa fuente (446 líneas, 0 discrepancias). El bloque del trigger (BLOQUE 5) no tenía el bug — ahí cada emoji aparece una sola vez — y se verificó igual por hex. **No** toca `emoji` en ningún punto, **no** borra columnas, `WHERE icono IS NULL` en los 3 UPDATE lo hace idempotente y no pisa un `icono` asignado a mano. **Cuando se aplique, marcar ✅ y anotar la verificación** (0 filas con `icono` NULL donde había emoji; `pg_get_functiondef` trae `icono` en el INSERT).
- ⚠️ `supabase_archivar_tarjetas.sql` — **BORRADOR, NO EJECUTADO (2026-09-07).** Modelo final del "borrado" de tarjetas: **archivar**, no borrar. (1) Agrega `tarjetas.archivada_en timestamptz` (nullable, sin default → toda tarjeta existente queda activa). (2) Índice parcial `tarjetas_activas_idx on tarjetas (user_id) where archivada_en is null`. (3) `drop view` + `create view tarjetas_con_deuda`: agrega el filtro `where archivada_en is null` y quita las 3 columnas del modelo de reasignación (`cantidad_gastos`, `total_pagado`, `total_gastado`) → vuelve a la forma original + filtro. (4) `drop function eliminar_tarjeta_usuario(uuid, uuid)`. (5) FK `movimientos.tarjeta_id`: **no se toca** (se deja en `ON DELETE RESTRICT`). Aditivo + limpieza; no toca datos de `movimientos` ni `tarjetas` (solo agrega la columna en NULL). **Cuando se aplique, marcar ✅ y anotar la verificación.**

**Nota (2026-09-04): este README no se actualizó desde el paso 21.** Los scripts de saldo calculado (`supabase_saldo_calculado.sql`), retiros (`supabase_tipo_retiro.sql`) y tarjetas de crédito (`supabase_tarjetas.sql`, `supabase_tarjetas_movimientos.sql`) ya están aplicados y viven en la app hoy (sus funcionalidades están activas), pero no fueron agregados al historial numerado de arriba. No los des por documentados acá — la regla de la nota de buena práctica sigue valiendo: la fuente de verdad es la base de datos.
