...
- ✅ `supabase_iconos_categorias.sql` — **APLICADO.** Fase B1 del `PLAN-iconos.md` (raíz): unificación visual de iconos (emoji → iconos de línea de lucide). **Aditivo puro:** `add column if not exists icono text` (nullable, sin default) en `categorias`, `movimientos` y `categorias_viaje`. **No** toca el trigger `handle_new_user()`, **no** borra `emoji`, **no** toca RLS ni grants. Confirmado ejecutado por el usuario en Supabase (mensaje de sesión, 2026-09-11) — no se re-verificó columna por columna desde esta sesión (sin acceso a la BD real), así que si algo se ve raro, correr la verificación comentada al final del script.
- ✅ `supabase_backfill_iconos.sql` — **APLICADO.** Fase B3 del `PLAN-iconos.md`: backfill de `icono` (agregada en B1) para las filas ya existentes de `categorias`, `movimientos` y `categorias_viaje`, más `create or replace` de `handle_new_user()` agregando la columna `icono` al INSERT de categorías por defecto. **Bug encontrado y corregido en la propia revisión, antes de ejecutarlo:** la primera versión comparaba `CASE emoji WHEN '🛒' THEN ...` con el emoji tipeado literal; el dry-run mostró que la mayoría caía en el fallback `'tag'`. Se diagnosticó con `encode(emoji::bytea, 'hex')` que el dato en la BD es UTF-8 canónico y correcto — el problema estaba en que varios de los ~450 literales de emoji del `.sql` (repetidos 6 veces en distintos bloques) se corrompieron al escribirse, no la BD. La versión que se ejecutó compara por **hex UTF-8** (`encode(emoji::bytea, 'hex')`, texto ASCII puro, inmune a ese problema); los pares hex→icono se generaron programáticamente desde `src/utils/mapaEmojiIcono.js` (no se retipeó ningún emoji a mano) y se verificaron con un script que compara cada línea del `.sql` contra el mapa fuente (446 líneas, 0 discrepancias) antes de correrlo. **No** toca `emoji` en ningún punto, **no** borra columnas. Confirmado ejecutado por el usuario en Supabase (mensaje de sesión, 2026-09-11) — no se re-verificó contra la BD real desde esta sesión.
...
- ✅ `supabase_fondo_emergencia_60meses.sql` — **APLICADO (2026-09-13).** Sube el techo de `fondo_emergencia.meses_meta` de 12 a 60 meses (5 años), acompañando el cambio de `LIMITE_MESES_META` en `Emergencia.jsx`. `drop constraint if exists` + `add constraint fondo_emergencia_meses_meta_rango_check check (meses_meta between 1 and 60)` sobre el constraint que agregó `supabase_reforzar_integridad.sql`. No toca el default (sigue en 6) ni ninguna fila existente (1-60 es superconjunto de 1-12, no había filas que pudieran violarlo). Confirmado por el usuario tras ejecutarlo en Supabase.
